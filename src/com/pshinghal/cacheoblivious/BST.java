...
// Source: http://algs4.cs.princeton.edu/32bst/BST.java
...
public class BST {
	...
	public int[] getArray() {
		ArrayList<Integer> list = new ArrayList<Integer>(inserted);
		
		addToArray(list, root);
	...
	private void addToArray(ArrayList<Integer> list, Node node) {
		if (node == null)
			return;
		addToArray(list, node.left);
		for (int i = 0; i < node.val; i++) {
			list.add(node.key);
		}
		addToArray(list, node.right);
	}
